...
\item \code{git push -u <repo name> <branch name>} upload the changes to \code{repo\_name} in the branch \textt{branch\_name}. \code{-u} means that (in future) typing \code{git push} will always use this host and branch.
\item \code{git pull <repo name> <branch name>} download the changes from the branch \textt{branch\_name} on \code{repo\_name}. If the remote repo is stored as default (or it's the only one), \code{git pull} will work as well.
\end {itemize}
