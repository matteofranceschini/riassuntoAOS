...
\begin{itemize}
	\item \code{git init .}, initialize a new repository in the current folder 
	\item \code{git add <file> <file>}, adds files to the commit
	\item \code{git commit -m"<comment>"}, commits the changes
	\item \code{git remote add <repo\_name> <repo\_url>}, adds a remote repository (default name: origin)
	\item \code{git push -u <repo\_name> <repo\_branch>}, pushes the changes to the remote repository and the \code{-u} tells git to remember repo name and branch for the next pushes
\end{itemize}
...
\begin{itemize}
\item \code{git checkout -b <new branch name>} or \code{git branch <new branch name>} create the new branch
...
\item \code{git branch -d <branch name>} deletes the branch, once it's merged with the master there's no need to keep it. The \textbf{forced} delete is done by using \code{-D}.
\end{itemize}
...
\begin{itemize}
	\item \code{git branch} shows all the branches.
\end{itemize}

\section{Undo Commands}
\begin{itemize}
\item \code{git rm <file_name>} removes phisically the file from the disk and from the repo
\item \code{git reset --soft}, unstage committed files (does not edit the files).
\item \code{git reset --hard}, delete changes to the last commit, editing the files.
